...
class MovimentoAdmin extends Admin
{
    ...
    protected function configureListFields(ListMapper $listMapper)
    {
        $listMapper
            ...
            ->add('procedencia_destino')
			->add('quantidade_movimentada', null, array('label'=>'Quantidade'))
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        $formMapper
            //->add('id')
			->with('Movimento de Produto', array("class" => "col-md-7"))
			/*
    ...
    protected function configureShowFields(ShowMapper $showMapper)
    {
        $showMapper
            ...
            ->add('quantidade_movimentada', null, array('label'=>'Quantidade'))
            ->add('existencia_antes', null, array('label'=>'Antes'))
            ->add('existencia_depois', null, array('label'=>'Depois'))
    ...
    public function prePersist($object) {
		...
		$funcionario = $this->getConfigurationPool()->getContainer()
				->get("doctrine")->getRepository("AppBundle:Funcionario")
        ...
        $produto = $this->getConfigurationPool()->getContainer()
						   ->get("doctrine")->getRepository("AppBundle:Produto")
						   ->find($object->getProduto()->getId());
						   
		$existencia = $produto->getExistencia();
		
		$tipo_movimento = $this->getConfigurationPool()->getContainer()
						   ->get("doctrine")->getRepository("AppBundle:TipoMovimento")
						   ->find($object->getTipoMovimento()->getId());
		
		$existencia_anterior = $existencia->getExistencia();
		$existencia_actual = $existencia_anterior + $object->getQuantidade()*$tipo_movimento->getValor();
		
		$object->setExistenciaAnterior($existencia_anterior);
		$object->setExistenciaActual($existencia_actual);
		$existencia->setExistencia($existencia_actual);
		$existencia->setData(new \DateTime());		
		
		$this->getConfigurationPool()->getContainer()
                    ->get("doctrine")->getManager()->persist($existencia);
